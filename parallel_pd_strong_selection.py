...
4. float: delta value
5, ... array floats: b_vals. values of b (prisoner's dilemma param) to run simulations for 
"""
game_str = sys.argv[1]
...
b_vals = np.array(sys.argv[5:],dtype=float)
c,DELTA = 1.0,float(sys.argv[4]) #prisoner dilemma params
...
def run_parallel(b,i):
    ...
    """
    rand=np.random.RandomState()
    history = run_simulation(simulation_decoupled_update_exp_fitness,l,timestep,timend,rand,DELTA,prisoners_dilemma_averaged,(b,c),save_areas=False)
