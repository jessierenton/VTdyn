...
def run_single(i,threshold_area_fraction,death_to_birth_rate_ratio):
    ...
    history = lib.run_simulation(simulation,L,TIMESTEP,TIMEND,rand,progress_on=False,
                init_time=INIT_TIME,til_fix=False,save_areas=False,
                return_events=False,save_cell_histories=True,N_limit=MAX_POP_SIZE,
                save_areas=True,domain_size_multiplier=domain_size_multiplier,
                rates=rates,threshold_area_fraction=threshold_area_fraction)
    outdir = PARENTDIR+'/DtoB%.1f_Thresh%.2ff'%(death_rate,threshold,domain_size_multiplier)
